<?php
	...
	class GT3_ElementorPhoto_Masonry extends Widget_Base {

		public function get_title(){
			return esc_html__('Masonry', 'gt3_elementor_photo_gallery');
		}
		...
		protected function render(){
			...
			if(isset($settings['slides']) && is_array($settings['slides']) && count($settings['slides'])) {
				...
				<div <?php $this->print_render_attribute_string('wrapper') ?>>
					...
					<?php } ?>
					<div class="isotope_wrapper gt3_clear " data-cols="<?php echo esc_attr($settings['cols']) ?>">
						<?php
					...
					<?php
						if((bool)($settings['show_view_all']) && count($load_more_images)) {
							if(empty($settings['view_all_link_text'])) {
								$settings['view_all_link_text'] = esc_html__('More', 'gt3_elementor_photo_gallery');
							}
