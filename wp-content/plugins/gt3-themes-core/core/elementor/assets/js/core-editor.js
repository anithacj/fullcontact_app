		...
		initialize: function () {
			...
			window.elementor.hooks.addAction( 'panel/open_editor/column', function( panel, model, view ) {
				...
				window.elementor.channels.editor.on('change:column', function (sectionName, editor) {
					...
					if (sectionName.elementSettingsModel.attributes.gt3_tabs && sectionName.elementSettingsModel.attributes.gt3_tabs  === 'yes') {
						...
						view.$el.addClass('gt3_column_tabs-elementor');
						if (settings['gt3_tabs_type'] === 'vertical') {
							view.$el.addClass('gt3_column_tabs-type_vertical');
						}else{
							view.$el.removeClass('gt3_column_tabs-type_vertical');
						}
						view.$el.attr('data-settings',JSON.stringify(settings));
						view.$el.attr('data-tab-items',JSON.stringify(tab_items));
					}else{
						view.$el.removeClass('gt3_column_tabs-elementor');
						view.$el.removeClass('gt3_column_tabs-type_vertical');
						view.$el.removeClass('gt3_column_tabs__activated');
					}
