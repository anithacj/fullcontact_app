	...
	switch($settings['icon_image']) {
		...
		case 'image':
			if(isset($settings['image']['id']) && (bool) $settings['image']['id']) {
				...
				if($image) {
					$icon = '<div class="icon_container" '.$image_style.'><img src="'.aq_resize($image['0'], (int)$settings['image_size']['width']*2, (int)$settings['image_size']['height']*2, true, true, true).'" alt="" /></div>';
				} else {
